...
:tanzu-jms-docs: https://techdocs.broadcom.com/us/en/vmware-tanzu/data-solutions/tanzu-rabbitmq-oci/4-3/tanzu-rabbitmq-oci-image/site-jms.html

== Introduction

RabbitMQ server together with this RabbitMQ JMS client library provides a JMS provider implementation.
The JMS Client for RabbitMQ implements the JMS specification on top of the
...
To support the JMS Topic messaging models, RabbitMQ includes the
https://github.com/rabbitmq/rabbitmq-server/tree/main/deps/rabbitmq_jms_topic_exchange[RabbitMQ JMS Topic Exchange plugin].
...
Where a limitation does not apply to link:{tanzu-jms-docs}[VMware Tanzu RabbitMQ], which implements JMS
on top of AMQP 1.0 and the link:https://github.com/apache/qpid-jms[Qpid JMS client] instead of AMQP 0-9-1
and this client, this is noted below.
...
* Message selectors for topics are supported with the RabbitMQ JMS Topic Exchange plugin.
  ...
  (However, message selectors for both queues and topics are supported in Tanzu RabbitMQ,
  where they are evaluated on the broker.)
* The JMS queue browser implementation consumes messages from the queue making them temporarily unavailable
  ...
  (However, queue browsers are implemented correctly in Tanzu RabbitMQ: messages are inspected on the broker
...
* The JMS `NoLocal` parameter, which prevents delivery of
 ...
 (However, `NoLocal` is supported in Tanzu RabbitMQ 4.4 and later.)
* Client ID uniqueness is enforced only within a single JVM process.
 Unlike traditional JMS brokers, the RabbitMQ JMS Client validates client ID uniqueness
 ...
 (However, Tanzu RabbitMQ 4.4 and later enforces client ID uniqueness across the whole cluster.)
