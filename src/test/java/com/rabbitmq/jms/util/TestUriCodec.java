...
public class TestUriCodec {
    ...
    public void testDecUserinfo() {
        for (String[] arr : UI_TESTS) {
            assertEquals(arr[1], decUserinfo(arr[2], "UTF-8"), "Failed to encode "+arr[0]);
        }
    }
    ...
    public void testDecScheme() {
        for (String[] arr : SC_TESTS) {
            assertEquals(arr[1], decScheme(arr[2], "UTF-8"), "Failed to encode "+arr[0]);
        }
    }
    ...
    public void testDecSegment() {
        for (String[] arr : SEG_TESTS) {
            assertEquals(arr[1], decSegment(arr[2], "UTF-8"), "Failed to encode "+arr[0]);
        }
    }
    ...
    public void testDecHost() {
        for (String[] arr : HOST_TESTS) {
            assertEquals(arr[1], decHost(arr[2], "UTF-8"), "Failed to encode "+arr[0]);
        }
    }
    ...
    public void testDecPath() {
        for (String[] arr : PATH_TESTS) {
            assertEquals(arr[1], decPath(arr[2], "UTF-8"), "Failed to encode "+arr[0]);
        }
    }
